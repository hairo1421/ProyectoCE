...
autoplot.forecast <- function(forecast, forc_name, ts_object_name, 
  ...
  ggplot(df, aes(time, x)) +
    ...
    geom_line(colour = "#318ce7", size = 1) +
    geom_line(data=df[!is.na(df$forecast), ], aes(time, forecast), color="green", na.rm=TRUE) +
